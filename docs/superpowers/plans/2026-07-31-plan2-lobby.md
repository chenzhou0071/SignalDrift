...
git -C "E:\pro\unity(游戏)" commit -m "feat(lobby): 大厅消息ID、DTO与配置加载"
```
...
```go
type User struct{ UID int64; Username, PasswordHash string }
type Profile struct{ UID int64; Elo, MaxElo, Wins, Losses int }
...
type Store interface {
	...
	GetProfile(uid int64) (*Profile, error)                  // 不存在则返回初始档案(Elo 1000)并落库
	UpdateElo(uid int64, newElo int, won bool) error         // 同步更新 MaxElo/Wins/Losses
...
func (m *MemStore) CreateUser(username, hash string) (int64, error) {
	...
	m.nextUID++
	m.byName[username] = &User{UID: m.nextUID, Username: username, PasswordHash: hash}
...
func (m *MemStore) UpdateElo(uid int64, newElo int, won bool) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	p, ok := m.profiles[uid]
	if !ok {
...
```bash
git -C "E:\pro\unity(游戏)" add server/internal/store
git -C "E:\pro\unity(游戏)" commit -m "feat(store): Store接口与内存实现"
```
...
func TestMySQLProfileAndFriend(t *testing.T) {
	...
	if err := s.UpdateElo(a, 1016, true); err != nil {
		t.Fatal(err)
	}
	p, _ = s.GetProfile(a)
...
func (s *MySQLStore) GetUserByName(username string) (*User, error) {
	u := &User{}
	err := s.db.QueryRow("SELECT uid,username,password_hash FROM user WHERE username=?", username).
		Scan(&u.UID, &u.Username, &u.PasswordHash)
...
func (s *MySQLStore) GetProfile(uid int64) (*Profile, error) {
	p := &Profile{UID: uid}
	err := s.db.QueryRow("SELECT elo_score,max_elo,wins,losses FROM user_profile WHERE uid=?", uid).
		Scan(&p.Elo, &p.MaxElo, &p.Wins, &p.Losses)
	if err == sql.ErrNoRows {
		if _, err := s.db.Exec("INSERT INTO user_profile(uid) VALUES(?)", uid); err != nil && !isDup(err) {
			return nil, err
		}
		return &Profile{UID: uid, Elo: 1000, MaxElo: 1000}, nil
	}
	return p, err
}

func (s *MySQLStore) UpdateElo(uid int64, newElo int, won bool) error {
...
```bash
git -C "E:\pro\unity(游戏)" add server
git -C "E:\pro\unity(游戏)" commit -m "feat(store): MySQL实现与建表SQL、集成测试"
```
...
```bash
git -C "E:\pro\unity(游戏)" add server/internal/lobby
git -C "E:\pro\unity(游戏)" commit -m "feat(lobby): HMAC重连Token与ELO计算"
```
...
```bash
git -C "E:\pro\unity(游戏)" add server/internal/lobby
git -C "E:\pro\unity(游戏)" commit -m "feat(lobby): ELO匹配池——30秒放宽分差、取消、去重"
```
...
func (q *EventQueue) process(ev MatchResultEvent) {
	...
	if err := q.st.UpdateElo(ev.WinnerUID, nw, !ev.Draw); err != nil {
		log.Printf("ERROR eq update winner elo: %v", err)
	}
	if err := q.st.UpdateElo(ev.LoserUID, nl, false); err != nil {
...
注意：`Draw==true` 时 `UpdateElo(winner,...)` 的 won 传 `!ev.Draw`=false，胜负计数正确；平局双方均不加 Wins（losses 语义：Draw 时 loser 侧也传 false，会加 Losses——**修正**：MemStore/MySQL 的 `UpdateElo(uid,newElo,won)` 无法表达平局，扩展为 `UpdateElo(uid int64, newElo int, result int)`，result: 1 胜 / 0 平 / -1 负，胜加 wins、负加 losses、平不加。Task 2/3 的实现与测试同步用此签名（本计划以此签名为准，Task 2/3 直接按 result int 实现）。
...
```bash
git -C "E:\pro\unity(游戏)" add server/internal/lobby server/internal/store
git -C "E:\pro\unity(游戏)" commit -m "feat(lobby): 在线索引与异步事件队列(ELO结算/战绩入库)"
```
...
func (svc *Service) handleLogin(s *gateway.Session, f *protocol.Frame) {
	...
	reply(s, protocol.MsgLoginResp, LoginResp{
		Code: 0, UID: u.UID, Elo: p.Elo, Token: svc.tokens.Issue(u.UID),
	})
}
...
func (svc *Service) handleFriendList(s *gateway.Session, f *protocol.Frame) {
	...
	for _, id := range ids {
		...
		resp.Friends = append(resp.Friends, FriendInfo{
			UID: id, Elo: p.Elo, Online: svc.pres.IsOnline(id),
		})
	}
...
func (svc *Service) handleProfile(s *gateway.Session, f *protocol.Frame) {
	...
	reply(s, protocol.MsgProfileResp, ProfileResp{
		Code: 0, UID: p.UID, Elo: p.Elo, MaxElo: p.MaxElo, Wins: p.Wins, Losses: p.Losses,
	})
}
...
```bash
git -C "E:\pro\unity(游戏)" add server
git -C "E:\pro\unity(游戏)" commit -m "feat(lobby): 注册/登录/档案/好友 handlers 与鉴权中间件"
```
...
func (svc *Service) pollMatchesOnce() {
	for _, pair := range svc.pool.Poll() {
		...
		if sa, ok := svc.pres.Get(pair.UIDA); ok {
			reply(sa, protocol.MsgMatchFound, MatchFoundPush{RoomID: roomID, OpponentUID: pair.UIDB, OppElo: pair.EloB})
		}
		if sb, ok := svc.pres.Get(pair.UIDB); ok {
			reply(sb, protocol.MsgMatchFound, MatchFoundPush{RoomID: roomID, OpponentUID: pair.UIDA, OppElo: pair.EloA})
		}
...
```bash
git -C "E:\pro\unity(游戏)" add server/internal/lobby
git -C "E:\pro\unity(游戏)" commit -m "feat(lobby): 匹配handlers与撮合循环、房间分配注入点"
```
...
```bash
git -C "E:\pro\unity(游戏)" add server
git -C "E:\pro\unity(游戏)" commit -m "feat(lobby): 主进程集成——断连回调、匹配循环、优雅退出顺序"
```
...
1. **规格覆盖**（规格 6.1 大厅部分）：账号注册登录密码哈希(T7)、ELO 段位(T4,T6)、好友与在线状态(T7)、ELO 匹配池 30 秒放宽/可取消(T5,T8)、异步事件队列写 ELO/历史(T6)、重连 Token 签发(T4,T7)、数据库表(T3)。房间分配为注入点(T8)由计划 4 实现；Token 的"房间恢复"消费方也在计划 4。
2. **占位符扫描**：无 TBD；T6 中发现并修正了 `UpdateElo` 无法表达平局的问题（签名改为 result int，T2/T3 按修正后签名实现）。
3. **类型一致性**：`Store` 接口签名全计划统一；`recv[T]` 泛型 helper 需 Go 1.22 OK；`MsgErrorResp=299` 仅用于未登录/通用错误，业务应答用各自 OK 消息携带 code。
