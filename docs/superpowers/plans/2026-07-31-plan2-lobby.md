...
**Tech Stack:** Go 1.24+（go-sql-driver/mysql v1.10.0 与 edwards25519 v1.2.0 均要求 go ≥ 1.24，已实测 tidy 无法压回 1.22——零依赖时代的 1.22 约束随依赖引入失效）；新增依赖仅 `github.com/go-sql-driver/mysql`、`golang.org/x/crypto/bcrypt`。
...
- 战斗线程永不直接碰 MySQL：对局结算类写库（ELO/战绩）走异步事件队列；大厅 handler 的直接读写（注册/登录/档案建档等）属正常路径
...
- [x] **Step 1: 追加消息 ID**（modify msgid.go）
...
- [x] **Step 2: DTO**（`server/internal/lobby/dto.go`）
...
- [x] **Step 1: 写失败测试**（`memstore_test.go`）
...
- [x] **Step 2: 运行确认失败** → `go test ./internal/store/ -v` FAIL

- [x] **Step 3: 实现**（`store.go` 放接口与结构体如上；`memstore.go`）
...
- [x] **Step 1: 建表 SQL**（`schema.sql`，按规格第七章）
...
- [x] **Step 2: 集成测试**（`mysql_test.go`，首行 `//go:build integration`；复用 Task 2 的三个测试场景，改为对 `NewMySQL(os.Getenv("SD_TEST_DSN"))` 跑；每个测试用随机用户名避免脏数据）
...
- Produces: `lobby.NewService(cfg *config.LobbyConfig, st store.Store, pool *MatchPool, pres *Presence, eq *EventQueue, tokens *TokenIssuer) *Service`、`(*Service).Mount(r *gateway.Router)`（注册全部大厅 msgID）、`(*Service).OnSessionClosed(s *gateway.Session)`（Presence 解绑+匹配池移除，网关断连时调用）。内部规则：注册校验用户名 3-32 字符、密码 6-64 字符；登录成功 `s.UID=uid`、Presence.Bind、发 Token；重复登录踢旧会话；**AddFriend 需校验好友 UID 存在（GetUser），不存在回 404（防幽灵好友——无外键、Store 层不拦）；不允许加自己为好友，自加回 400**

- [x] **Step 1: 写失败测试**（走真实 Router 分发，Session 用 net.Pipe 构造；helper 读取 Session.SendQueue 解析 JSON 应答）
...
func TestFriendAddNotFound(t *testing.T) {
	_, r, m := testService(t)
	s := newSess(t, m)
	send(r, s, protocol.MsgRegisterReq, RegisterReq{Username: "n1", Password: "123456"})
	recv[RegisterResp](t, s, protocol.MsgRegisterResp)
	send(r, s, protocol.MsgLoginReq, LoginReq{Username: "n1", Password: "123456"})
...
- [x] **Step 2: 确认失败 → 实现**：Server 增加字段 `onClosed func(*Session)` 与 `SetOnSessionClosed`；`teardown` 末尾若非 nil 则调用。

- [x] **Step 3: main.go 完整装配**（替换 Echo 段）
